      ...
      <div className="crm-dots-chart">
        {sorted.map((d, i) => {
          ...
          return (
            <div
              key={d.day}
              className="crm-dot has-order"
              style={{
                width: 10 + intensity * 6,
                height: 10 + intensity * 6,
                background: `rgba(0,245,160,${0.3 + intensity * 0.7})`,
                boxShadow: `0 0 ${intensity * 12}px rgba(0,245,160,${intensity * 0.5})`
              }}
              title={`${d.day}: ${d.orders} pedidos`}
            />
          )
        })}
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.75rem', padding: '0 0.25rem' }}>
        <span style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.25)' }}>{sorted.length} dias</span>
        <span style={{ fontSize: '0.6rem', color: '#00f5a0', fontFamily: 'Inter, monospace' }}>{sorted.length} dias com pedidos</span>
      </div>
    </div>
  )
}
            ...
            return data.map((d, i) => {
              ...
              const el = (
                <circle
                  ...
                  strokeDasharray={`${dash} ${251 - dash}`} strokeDashoffset={-acc}
