          ...
          <div style={{ padding: '2rem 1.75rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            {product.tags?.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                {product.tags.map(tag => <span key={tag} style={{ padding: '0.25rem 0.75rem', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: '100px', fontSize: '0.7rem', fontWeight: '500', color: 'var(--text-muted)' }}>{tag.trim()}</span>)}
              </div>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: 'auto' }}>
              ...
              {settings.store_whatsapp && (
                <a href={whatsappLink} target="_blank" rel="noopener" className="whatsapp-btn" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.6rem', width: '100%', padding: '0.9rem', background: 'rgba(37,211,102,0.1)', color: '#25d366', fontSize: '0.9rem', fontWeight: '700', border: '1px solid rgba(37,211,102,0.3)', borderRadius: 'var(--radius-sm)', letterSpacing: '0.02em' }}>
